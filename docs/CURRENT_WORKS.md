...
- **eval suite for the skill** — `waza check` flags "No eval.yaml". `waza init` / `waza new eval` scaffolds tests; not yet done.
- **`recipe-book.md`** — still the hand-curated placeholder; needs initial intent→rule content.
- **Release prep** — README, CI workflows (ADR-0006 describes the intended GitHub Actions: CI on push/PR, weekly Infection), and `composer.json` `description`/keywords are set but not yet validated against a real `composer install` from a clean checkout.
